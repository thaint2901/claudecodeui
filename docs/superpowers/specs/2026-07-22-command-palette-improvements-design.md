...
**Built-ins by subtraction (amended after final review):** `slash_commands` from `system/init` is not a pure built-in list — per Claude Code's own docs it "includes built-in commands plus bundled skills," and in practice this device shows ~150 entries including every installed user/project/plugin skill. Merging `data.claudeBuiltIn` wholesale (excluding only the 6 ccui pseudo-command names) would therefore duplicate almost every skill into both the "Claude Code Built-in" group and its project/user group. The fix: in the frontend merge (`useSlashCommands.ts`, the one place all three sources converge), the claude-builtin group is computed as `data.claudeBuiltIn` minus every name already known from a richer source — `data.builtIn` (ccui), the skills scan, and `data.custom`. Each name then lands in exactly one group, its most informative one, and the set is self-maintaining: installing a new skill removes it from claude-builtin automatically, and a genuinely new CLI built-in appears there without any code change.
