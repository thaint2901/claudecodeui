...
- **The local dev server is a systemd user service (`cloudcli-dev`), serving the MAIN checkout on 5173/3001.** It auto-starts with WSL (linger on) and restarts on crash — never kill it manually or start a competing `npm run dev` on the same ports. Worktree changes reach it via `cp <worktree>/<file> <main>/<file>` (frontend = HMR picks it up; sync ALL related files — a partial copy creates a phantom-bug hybrid). To test a whole branch instead, run a second instance from the worktree: `SERVER_PORT=3002 VITE_PORT=5174 npm run dev` → browse 5174, leave 5173 untouched. The `tmp.md` and `.playwright-mcp/` directories in the project root are byproducts of past sessions — ignore them.
...
- **Every new non-message WebSocket `kind` must `return` early in `useChatRealtimeHandlers.ts`'s FIRST switch.** The `default:` branch merely `break`s — the hazard is *after* it: an allow-by-default `shouldPersist` guard force-casts anything not explicitly excluded into `NormalizedMessage` and calls `sessionStore.appendRealtime()`, so an event with no `.id` corrupts that session's store and crashes every later merge (`.id.startsWith` on `undefined`). Follow `session_lock_state_changed`; also add the kind to `isNonTranscriptKind` (`src/components/chat/utils/realtimeKinds.ts`) so a future refactor cannot silently re-open the hole. To show something in the transcript, append your own well-formed message with an explicit `.id`, as `case 'protocol_error'` does.
...
- **Agent tool_result content is an array of text blocks whose LAST block is runtime metadata** (`agentId: ... <usage>...</usage>`) — never join blocks raw; use `extractSubagentText()` from `src/components/chat/utils/subagentToolNames.ts`. **The block count depends on whether the subagent ran in the foreground**, and that in turn depends on env: a foreground subagent yields **2 blocks** `[answer, metadata]`, so the first block equals its final assistant message and the Result box dedupes against the drawer. A backgrounded or fork subagent yields **1 block** — a launch stub (`Async agent launched…`) with no answer, and no later `tool_result` arrives to replace it (measured: none within 40s). `CLAUDE_CODE_FORK_SUBAGENT=1` is what flips normal subagents to the 1-block form, which is why turning that flag on session-wide would strip the inline answer from every subagent's Result box. `isSubagentMetadataBlock()` (`subagentToolNames.ts:20`) matches `/^agentId: /` and so does not match the stub — the drawer is unaffected either way, since it keys off `toolUseResult.agentId` (`claude-sessions.provider.ts:188`), not the block count.
- **SDK `forkSession: true` semantics:** the fork transcript is a byte-copy of the parent's history rewritten to the fork's sessionId with NO field pointing back to the parent; inherited subagent files stay under the PARENT's `<session-id>/subagents/` (hence `resolveAgentFilePath`'s sibling-dir fallback); and the stream announces ONLY the fork's NEW id — the one case where the provider session id changes mid-stream (`recaptureForkSession` in claude-sdk.js).
- **The sessions-watcher re-derives `custom_name` from transcript title events on every sync** — an app-set name (e.g. "X (fork)") WILL be overwritten unless a `custom-title` event exists in the transcript itself; write it via `renameSessionById` (which calls SDK `renameSession`) as soon as the provider id is announced, not after the run.
- **Native CLI local commands (`/compact`, `/subtask`… typed in the terminal) persist as `type: "system", subtype: "local_command"` entries with NO `message` field** — ccui's normalizer (claude-sessions.provider.ts, keys off `message.role`) drops them, so CLI-run sessions viewed in ccui are missing their slash-command lines. Known gap, deliberate.
- **`sessions.created_at` mixes two formats** (ISO `2026-07-23T07:18` from watcher vs space `2026-07-23 07:30` from app inserts) — `ORDER BY created_at` string-compares across them incorrectly (`'T' > ' '`), so "latest rows" queries silently mis-sort. Filter by exact condition instead.
- **Frontend tests that import the component chain (ToolRenderer → Markdown → react-syntax-highlighter) crash under `npx tsx --test`** (ESM named-export interop) — only leaf utils under `src/components/chat/utils/` are runnable; extract logic there to test it (e.g. `transcriptEndsWithText`). `SubagentContainer.test.ts` typechecks but cannot run.
- **Running the whole test tree takes two commands, and the glob matters.** Server: `npx tsx --test --experimental-test-module-mocks --tsconfig server/tsconfig.json "server/**/*.test.ts" "server/**/*.test.js"` — quote the globs; an unquoted or narrower one silently reports a smaller count, which is how a wrong "341 tests" once reached a report. Frontend leaf utils are NOT covered by that and need `npx tsx --test "src/components/chat/utils/*.test.ts"`. `npm run lint` never reaches 0 warnings here, so capture the count before your change and compare — a raw number in a report means nothing without its baseline.
- **Three traps when writing a one-off probe script.** (1) `npx tsx --test -e "<code>"` runs the **whole discovered suite** instead of your inline code — write a file. (2) The file must live **inside the worktree**, not in a scratch dir like `$CLAUDE_JOB_DIR/tmp`, or Node cannot resolve `node_modules` (`ERR_MODULE_NOT_FOUND: @anthropic-ai/claude-agent-sdk`); write it in, run it, `rm` it. (3) The SDK's `query.close?.()` returns `undefined`, not a promise — `await`ing it directly throws `Cannot read properties of undefined`; guard with `const c = q.close?.(); if (c?.then) await c;`. `spikes/streaming-input-mode/_probe-lib.mjs` already handles all three.
- **Killing a manually-started dev instance (e.g. the worktree's 5174/3002 one): kill each child pid (`concurrently`, `tsx`, `vite`) with `kill -9` individually** — killing the npm wrapper leaves orphans holding the ports. (The main 5173 instance is systemd-managed — use `systemctl --user`, not kill.) In Playwright, a cold deep-link to `/session/<id>` lands on "Choose Your Project" — click through the sidebar (or navigate again after the project context loads).
- **`claude-models.provider.ts`'s Claude model list is hand-maintained, not synced from any API.** A prior attempt to fetch it live via the Agent SDK's `query().supportedModels()` was abandoned — it spins up a real session that writes a jsonl file misattributed to the invoking workspace. The public Anthropic Models API (`/v1/models`) can't substitute either — it's a different taxonomy (raw model IDs, no CLI aliases like `sonnet[1m]`/`opusplan`/`best`, no account-tier-dependent `default`). The only source of truth is `code.claude.com/docs/en/model-config` — re-check its alias table AND its per-model effort-tier table (e.g. Sonnet 5 supports `xhigh`, Sonnet 4.6 doesn't) whenever Anthropic ships a new model generation.
- **Check `src/components/provider-auth/hooks/useProviderAuthStatus.ts` before writing new provider-auth-status fetch logic** — it already fetches `/api/providers/:provider/auth/status` for all 4 providers with per-provider failure isolation and a fail-closed default (used by Settings/Onboarding). A from-scratch reimplementation is an easy way to end up both duplicative and weaker (e.g. an un-isolated `Promise.all` where one provider's failure fail-opens all four).
- **`gh pr merge --delete-branch` run from a feature-branch worktree fails locally** (`fatal: 'main' is already used by worktree at ...`) when the primary checkout has `main` checked out — the merge itself still succeeds on GitHub (verify via `gh pr view --json state,mergedAt`). Fast-forward the primary checkout manually (`git merge --ff-only origin/main`) and delete the remote branch separately (`git push origin --delete <branch>`) instead.
- **`react-syntax-highlighter` builds one React element per token — ~10 spans per line of code** (measured: 2026 spans for 5 blocks; ChatGPT's CodeMirror-based view emits 2.83/line for the same shape). Tokenizing is not the cost — `matchGrammar` is 4.2ms while `createElement` is 425ms — so *any* re-render that reaches a message row re-pays the whole thing. Before the memo boundaries went in, one keystroke on a 5-code-block session cost **1456ms INP** and produced **zero DOM mutations**. The defenses are `React.memo` on `Markdown` and on `HighlightedCode` (keyed by `raw`/`language`/`isDarkMode`); removing either brings the stall straight back.
- **Every prop handed to a message row must be referentially stable.** A single inline arrow in `ChatInterface` (`onEditPrompt={(m) => …}`) defeated `React.memo` on the entire rendered transcript — that was a real regression shipped in the edit-prompt fork work. `react/jsx-no-bind` is enabled via file-level eslint comments in `ChatInterface.tsx`, `ChatMessagesPane.tsx`, `MessageComponent.tsx`, and `ToolGroupContainer.tsx` to catch it (inline rather than in `eslint.config.js`, which a config-protection hook blocks). Host elements are exempt — they have no memo boundary to break.
- **Perf numbers on the Vite dev server run ~2× the production build** (StrictMode double-render; measured 1.87–2.01× across four different interactions, with nothing else hiding in dev). Always confirm a fix on `npm run build` output before believing the magnitude. Separately, **turning on a Chrome performance trace inflates native frames 2.6–3×** — a traced `removeChild` read as 68–81ms where the untraced call was 0.2–1.3ms. Untraced in-page instrumentation is the authoritative number; use traces for *proportions* only.
- **Typing measured via `native value setter` + synthetic `input` event reads ~3× faster than real keystrokes** (474ms vs 1456ms on the same page), because real key events queue behind the long task they trigger. Use `Input.dispatchKeyEvent` (or a real Playwright `type`) whenever the absolute number matters. `npm run perf:budget -- --port=<app-port>` drives real key events over CDP against an already-open Chrome (`google-chrome --remote-debugging-port=9222`) and fails the documented budgets: INP < 200ms, no single long task > 50ms. **The port is required** — "first tab on localhost" could pick 3001, which serves a possibly weeks-stale prebuilt `dist/`. The run also aborts as `NOT MEASURED` (exit 2) when the page has no code blocks or a probe observes zero frames, because a page with nothing to highlight is an unmeasured page, not a fast one.
...
- **`queryClaudeSDK(command, options, ws)`'s `options.sessionId` is the PROVIDER-native session id, not the app session id.** Anything keyed per-session must use `options.appSessionId` (threaded separately through `chat-websocket.service.ts` at both `spawnFn` call sites): forks change the provider id mid-stream via `recaptureForkSession`, and a brand-new session has no provider id until its first message announces one.
- **Anything set on `process.env` after `mapCliOptionsToSDK` builds `sdkOptions.env` never reaches the CLI.** `sdkOptions.env = { ...process.env }` is a snapshot, and the SDK's `initialize()` spawns with `env: {...process.env}` of its own — so late mutation is dead config. This is why `CLAUDE_CODE_STREAM_CLOSE_TIMEOUT` was a no-op since 2026-04 (and the name exists in neither `sdk.mjs` nor CLI 2.1.220 — only a stale `sdk.d.ts` comment). Set provider env inside `sdkOptions.env`, never on `process.env`.
- **Three test files fail when the suite is run from inside a Claude Code session, on `main` too.** `server/tests/claude-sdk-options.test.js` asserts `sdkOptions.env.CLAUDE_CODE_FORK_SUBAGENT === undefined`, but `mapCliOptionsToSDK` spreads `process.env` and the ambient session sets it to `1`; the two `renameSessionById` cases in `server/modules/providers/tests/rename-session.service.test.ts` are the documented `CLAUDE_CONFIG_DIR` sensitivity. Baseline before blaming your branch: run the same files from the main checkout.
- **Streaming input mode unlocked the SDK `Query` control methods** — `backgroundTasks(toolUseId?)` (the Ctrl-B action; returns `Promise<boolean>`, it is NOT a query for the running-task list), `stopTask(id)` (emits a `task_notification` with `status: 'stopped'`), `setPermissionMode()`, `setModel()`, `applyFlagSettings()`. All are streaming-only. Two traps: successive `applyFlagSettings` calls **replace** the whole `permissions` object rather than merging, so always send the complete layer and clear it with `permissions: null`; and `applyFlagSettings`' `effortLevel` is a NARROWER domain than the `effort` query option (`'low'|'medium'|'high'|'xhigh'` vs those plus `'max'` and numbers), so effort cannot be fully reconciled on a running process.
- **An interrupted turn DOES emit a terminating `result`** — `subtype: 'error_during_execution'`, within milliseconds (`spikes/streaming-input-mode/interrupt-result.mjs`). An earlier spike concluded the opposite and that claim reached this file, the design spec, and a comment in `claude-sdk.js`; it was an artifact of interrupting when no turn was actually in flight. So the pool must let that `result` settle the turn rather than settling it blind — settling early vacates the turn slot while the CLI is still emitting, and `SDKResultMessage` carries **no turn-correlation field** (only `uuid`/`session_id`/`num_turns`), so a frame that arrives after the slot is reused cannot be attributed. Keep the slot until the real terminator lands, with a timed fallback for the case where `interrupt()` itself fails.
- **A settling background task announces itself TWICE, and only the second frame carries the output path.** `task_updated` with a terminal `patch.status` arrives first, then `task_notification` ~0–1 ms later (measured for `completed` and `failed`; `killed` is unforceable from a probe). Only `task_notification` carries `output_file`. Two traps follow: dedup that reports the *first* frame downgrades every ordinary task's row to "output could not be located"; and if the first frame clears your per-task bookkeeping, the second one arrives with no owner — which is how per-user delivery of `background_task` was silently defeated for the ordinary case while all its tests passed.
- **A test fake for the SDK must be checked against a real-CLI probe at least once per message shape the logic depends on.** The double-announce above went undetected through six task reviews and a green suite because every fake omitted the `task_updated` frame that the real CLI always sends. A green suite proves the code matches the fake's assumptions, not the CLI's. `spikes/streaming-input-mode/_probe-lib.mjs` exists for exactly this and forces an `inconclusive` branch so a probe that never exercised its condition cannot be read as a result.
- **Before adding a mechanism to `server/claude-session-pool.js`, write down its assumption and check it against every mechanism already there.** All three Criticals of the 2026-07-29 review round were two individually-reasonable assumptions that were mutually exclusive in composition — e.g. the abort fallback exists because the CLI may ack an interrupt and emit nothing, while the mechanism that fallback armed assumed the terminator was still coming. None was visible to task-scoped review; each surfaced only at the whole-branch review. The file holds lifetime state with no ground truth to check against (the SDK exposes no turn-correlation field, no running-task list — `backgroundTasks()` is the Ctrl-B *action* returning `Promise<boolean>` — and no signal that a process still serves a given conversation), so every mechanism here is a constrained guess and each new one adds a pair that can collide.
- **The `gh` active account can flip mid-session, not just between sessions.** It was verified as the write-capable account early in a session and was found switched to the dormant read-only one hours later, with subagents having made `gh` calls in between. Guard every `gh` write with an inline check (`gh auth status | grep -B2 "Active account: true"`) rather than checking once at the start.
- **The CLI resolves env from THREE sources, and `options.env` is only one.** Parent `process.env`, `options.env`, and the `env` block of the `settings.json` files named by `settingSources` (ccui ships `['project','user','local']`, `claude-sdk.js:262`). `options.env` can ADD what `settings.json` lacks but **cannot REMOVE what `settings.json` sets** — so a feature flag can be live on one host and absent on another with identical code (`CLAUDE_CODE_FORK_SUBAGENT` is in the dev box's `~/.claude/settings.json` but not a30's, and the systemd unit passes only `CLAUDE_CLI_PATH`). When measuring anything env-dependent, neutralise BOTH layers: delete the key from the env you pass AND drop `'user'` from `settingSources`. Doing one and not the other is how two spike "findings" were recorded wrong; `spikes/streaming-input-mode/_probe-lib.mjs` encodes the recipe.
- **`origin/main` can sit far behind local `main`** (63 commits, as of 2026-07-28 — the pre-squash history of PR #5 that GitHub squashed into `b9a6f50`). A PR then lists those commits too — 92 instead of 29 — even though the net diff is identical against either base. Verify with `diff <(git diff main HEAD) <(git diff origin/main HEAD)` before assuming the branch is polluted; the fix is fast-forwarding `origin/main`, which only the repo owner should push.
