...
- **Agent tool_result content is an array of text blocks whose LAST block is runtime metadata** (`agentId: ... <usage>...</usage>`) — never join blocks raw; use `extractSubagentText()` from `src/components/chat/utils/subagentToolNames.ts`. The first block always equals the subagent's final assistant message (the drawer transcript already shows it — that's why the Result box dedupes).
...
- **Streaming input mode unlocked the SDK `Query` control methods** — `backgroundTasks()` (the Ctrl-B equivalent), `stopTask(id)`, `setPermissionMode()`, `setModel()`, `applyFlagSettings()`. All are streaming-only. Two traps: `interrupt()` emits **no `result`**, so the awaiting turn must be settled explicitly or the run hangs in "processing"; and successive `applyFlagSettings` calls **replace** the whole `permissions` object rather than merging, so always send the complete layer and clear it with `permissions: null`.
