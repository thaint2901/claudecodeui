...
server/                # Express + ws backend (mostly ESM JS, migrating to TS)
  ...
  modules/              # NEW module-per-feature layout (replacing top-level routes):
    browser-use/          # service + REST + MCP routes; now has its own index.ts barrel (added phase 6)
    database/             # better-sqlite3 connection, init-db, migrations, schema, repositories
    events/                # leaf broadcast seam between services and the websocket hub (setBroadcastHandler/broadcast) — imports nothing from other modules
...
- **Backend module boundaries are enforced by ESLint.** `eslint.config.js` uses `eslint-plugin-boundaries` to treat each `server/modules/*` folder as one element. Cross-module imports must go through that module's barrel (`index.ts`/`index.js`) — direct deep imports fail lint. Backend modules may only `import type` from `server/shared/types.ts` or `server/shared/interfaces.ts`; runtime imports of shared utils must come from `server/shared/{http,workspace-paths,messages,json}.ts`, `frontmatter.ts`, or `claude-cli-path.ts` (the old god module `server/shared/utils.ts` — 1,286 LOC, 36 exports — was split by ownership and deleted in phase 6, no re-export shim; see ADR-0005). Legacy runtime files still permitted during the migration window: `server/utils/runtime-paths.js`. Enforcement now covers the legacy tier too (`backend-legacy` element added 2026-08-03: `server/routes/**`, root `server/*.{js,ts}`, `server/middleware/**`, `server/services/**`, `server/utils/**`, `server/constants/**`, `server/tests/**`) — a deep import from `routes/` into module internals fails lint.
- **`shared/wire-types.ts` (repo root) is the single source** for `MessageKind`/`GatewayEventKind`/`ServerEventKind`/`NormalizedMessage`. `server/shared/types.ts` and `src/stores/useSessionStore.ts` are facades that re-export from it. The realtime dispatch switches in `useChatRealtimeHandlers.ts` are exhaustive over these unions with `assertNever` in each `default` — adding a kind without a matching `case` is a compile error, not a runtime bug (see ADR-0005).
...
- **`npm test` / `npm run test:server` / `npm run test:client` / `npm run test:coverage` exist and run the `node:test` suites; vitest is NOT a dependency.** Tests that use `mock.module` need `--experimental-test-module-mocks` (Node 24) or they crash with `TypeError: mock.module is not a function`.
...
- **Every WebSocket `kind` needs an explicit `case` in `useChatRealtimeHandlers.ts`'s switch — enforced by the compiler since phase 6, not just convention.** Before the fix, an unhandled kind fell through to `default`, force-cast the raw event into `NormalizedMessage`, and called `sessionStore.appendRealtime()` — a non-chat event with no `.id` field corrupted that session's message store and crashed every later merge (`.id.startsWith` on `undefined`). Both switches are now exhaustive over `shared/wire-types.ts`'s unions with `assertNever` in `default`, so adding a kind without a case fails `npm run typecheck` instead of corrupting state at runtime.
...
- **Frontend tests that import the component chain (ToolRenderer → Markdown → react-syntax-highlighter) crash under `npx tsx --test`** (ESM named-export interop) — any client test that avoids importing that chain is runnable: leaf utils under `src/components/chat/utils/` (e.g. `transcriptEndsWithText`), hooks like `useChatMessages.test.ts`, and standalone renderers like `QuestionAnswerContent.test.tsx`. `SubagentContainer.test-disabled.ts` typechecks but cannot run.
