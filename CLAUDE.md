...
- **Restarting the dev server: kill each child pid (`concurrently`, `tsx`, `vite`) with `kill -9` individually** — killing the npm wrapper leaves orphans holding the ports. In Playwright, a cold deep-link to `/session/<id>` lands on "Choose Your Project" — click through the sidebar (or navigate again after the project context loads).
