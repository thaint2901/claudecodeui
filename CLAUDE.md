...
- **Browser auth token lives in `localStorage['auth-token']`.** Use it as `Authorization: Bearer <token>` for `curl` against the local dev server. `node:test` against pure helpers (no `@/` imports) is the only test path that runs without alias resolution — `vitest run` is broken in this checkout because there's no vitest config resolving the `@/` alias.
